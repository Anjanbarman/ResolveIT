...
class ComplaintServiceTests {
    ...
    void setup() {
        ...
        admin = userRepository.save(
                User.builder().name("Admin").email("admin@test.local").password("x").role(UserRole.ADMIN).build());
    ...
    void citizenCanReopenResolvedComplaint() throws Exception {
        ...
        Complaint reopened = complaintService.reopenComplaint(c.getId(), citizenAuth);
